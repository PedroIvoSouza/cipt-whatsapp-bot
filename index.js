...
// Função para enviar vCard com fallback SE não entregue
...
async function startBot() {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      await enviarEmail("CIPT Bot - Desconexão", "O bot do WhatsApp foi desconectado.");
    ...
    if (!msg.key.fromMe && msg.message?.conversation) {
      ...
      try {
        ...
        if (pergunta.includes("auditório") && pergunta.includes("sala de reunião")) {
          respostaExtra = "\n\nPercebi que você mencionou tanto o auditório quanto as salas de reunião. Vou esclarecer as diferenças: o auditório é voltado para grandes eventos e exige ofício prévio, enquanto as salas de reunião são exclusivas para permissionários e podem ser agendadas via recepção.";
        } else if (pergunta.includes("auditório")) {
          respostaExtra = "\n\nSe desejar, posso colocá-lo diretamente em contato com o responsável pelo auditório.";
        } else if (pergunta.includes("sala de reunião")) {
          respostaExtra = "\n\nAs salas de reunião são exclusivas para permissionários. Caso queira, posso enviar o contato da recepção para agendamento.";
        }
        ...
        const completion = await client.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: `Você é o assistente virtual do CIPT. Responda formal e simpático, usando Regimento Interno, fontes oficiais e contexto do CIPT.
...
"Não encontrei informações específicas sobre isso em nosso regimento interno e nem nas bases que eu uso para te responder. Você pode entrar em contato com a gestão do CIPT pelo e-mail supcti@secti.al.gov.br ou na recepção, tenho certeza de que lá você conseguirá tirar todas as suas dúvidas."`
            ...
            { role: "user", content: pergunta + respostaExtra }
          ],
          temperature: 0.3,
          max_tokens: 500
        });
